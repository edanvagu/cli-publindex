...
describe('validateBatch - casos válidos', () => {
  ...
  it('acepta múltiples artículos válidos', () => {
    const articles = [validArticle({ _fila: 2 }), validArticle({ _fila: 3, titulo: 'Otro título bastante largo' })];
...
describe('validateBatch - concordancia entre campos', () => {
    ...
    const result = validateBatch(
      [
        validArticle({
          gran_area: 'Humanidades',
          area: 'Historia y Arqueología',
          subarea: 'Biología Celular y Microbiología',
        }),
      ],
      [],
    );
  ...
  it('acepta subarea correcta', () => {
    const result = validateBatch(
      [validArticle({ gran_area: 'Humanidades', area: 'Historia y Arqueología', subarea: 'Historia' })],
      [],
    );
  ...
  it('rechaza fecha_aceptacion anterior a fecha_recepcion', () => {
    const result = validateBatch([validArticle({ fecha_recepcion: '2026-03-01', fecha_aceptacion: '2026-01-15' })], []);
    const err = result.errors.find((e) => e.field === 'fecha_aceptacion');
    expect(err).toBeDefined();
  });

  it('acepta fecha_aceptacion posterior a fecha_recepcion', () => {
    const result = validateBatch([validArticle({ fecha_recepcion: '2026-01-15', fecha_aceptacion: '2026-03-01' })], []);
    expect(result.errors.filter((e) => e.field === 'fecha_aceptacion')).toEqual([]);
  });

  it('rechaza pagina_final <= pagina_inicial', () => {
    const result = validateBatch([validArticle({ pagina_inicial: '10', pagina_final: '5' })], []);
    const err = result.errors.find((e) => e.field === 'pagina_final');
    expect(err).toBeDefined();
  });

  it('acepta pagina_final > pagina_inicial', () => {
    const result = validateBatch([validArticle({ pagina_inicial: '1', pagina_final: '15' })], []);
    expect(result.errors.filter((e) => e.field === 'pagina_final')).toEqual([]);
  });

  it('rechaza idioma igual a otro_idioma', () => {
    const result = validateBatch([validArticle({ idioma: 'Español', otro_idioma: 'Español' })], []);
    expect(result.errors.some((e) => e.field === 'otro_idioma')).toBe(true);
  });

  it('acepta idiomas diferentes', () => {
    const result = validateBatch([validArticle({ idioma: 'Español', otro_idioma: 'Inglés' })], []);
...
describe('validateBatch - enums y formatos', () => {
  ...
  it('acepta eval_interna con T o F', () => {
    expect(
      validateBatch([validArticle({ eval_interna: 'T' })], []).errors.filter((e) => e.field === 'eval_interna'),
    ).toEqual([]);
    expect(
      validateBatch([validArticle({ eval_interna: 'F' })], []).errors.filter((e) => e.field === 'eval_interna'),
    ).toEqual([]);
  });
