...
import { parsePublication, parseArticle, ojsArticuloToRow, extraerPublicationsXml, detectarPaginasNoEstandar } from '../../src/data/ojs-importer';
...
describe('extraerPublicationsXml', () => {
  it('extrae cada bloque <publication> como string independiente', () => {
    const bloques = extraerPublicationsXml(PUBLICATION_DOS_EN_ROOT);
    expect(bloques).toHaveLength(2);
    expect(bloques[0]).toContain('<title locale="es_ES">Título del artículo en español</title>');
    expect(bloques[1]).toContain('<title locale="es_ES">Solo título</title>');
  });

  it('retorna arreglo vacío si no hay publications', () => {
    expect(extraerPublicationsXml('<root><otro/></root>')).toEqual([]);
  });
});
...
describe('detectarPaginasNoEstandar', () => {
  it('retorna índices de artículos con paginasRaw presente', () => {
    ...
    expect(detectarPaginasNoEstandar(articulos)).toEqual([
...
describe('ojsArticuloToRow', () => {
  it('mapea los campos auto-rellenables al shape ArticuloRow', () => {
    const art = parsePublication(PUBLICATION_COMPLETO);
    const row = ojsArticuloToRow(art);
