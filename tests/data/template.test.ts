...
import { generarPlantillaConDatos } from '../../src/data/template';
...
function leerHojaArticulos(filePath: string) {
  const wb = XLSX.readFile(filePath, { cellStyles: true });
  return wb.Sheets['Artículos'];
}

function colorFondo(cell: any): string | undefined {
  return cell?.s?.fgColor?.rgb ?? cell?.s?.fill?.fgColor?.rgb;
}

describe('generarPlantillaConDatos — resaltado de obligatorios vacíos', () => {
  it('resalta en amarillo las celdas obligatorias sin datos', () => {
    const parcial: Partial<ArticuloRow> = {
    ...
    generarPlantillaConDatos([parcial], out);

    const ws = leerHojaArticulos(out);
    // Fila 2 en Excel = índice 1 (fila 0 es header)
    ...
    expect(colorFondo(urlCell)).toBe('FFEB9C');
  ...
  it('no resalta celdas de campos no obligatorios aunque estén vacías', () => {
    ...
    expect(colorFondo(doiCell)).toBeUndefined();
  });
});
